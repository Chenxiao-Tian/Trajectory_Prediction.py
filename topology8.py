# -*- coding: utf-8 -*-
"""
Created on Fri Jun 14 15:12:15 2024
            ...
            for i in range(start_idx, end_idx + 1):
                if i == start_idx:
                    ...
                    if data[start_idx, 2] == 0:
                        diffusion = sigma * 1 * np.sqrt(dt) * np.random.normal(size=2)
                    else:
                        diffusion = sigma * 1 * np.sqrt(dt) * np.random.normal(size=2)
...
true_segment_lengths = calculate_segment_lengths(true_path, segments)  # Use original data for true segment lengths
