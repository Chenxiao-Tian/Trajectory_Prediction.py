# -*- coding: utf-8 -*-
"""
Created on Thu Jun 27 15:10:37 2024
...
# -*- coding: utf-8 -*-
"""
Created on Wed Jun 26 15:30:03 2024
...
def create_lstm_model(input_shape, output_shape):
    model = tf.keras.Sequential([
        ...
        tf.keras.layers.TimeDistributed(tf.keras.layers.Dense(np.prod(output_shape), activation='linear')),
...
def train_and_predict(train_data, test_data, scaler, epochs=20):
    ...
    # 准备训练数据
    X_with, y_with = prepare_data_for_lstm(train_with_passenger, scaler)
    X_without, y_without = prepare_data_for_lstm(train_without_passenger, scaler)
    ...
    # 创建和训练模型
    model_with = create_lstm_model(input_shape, output_shape_with)
    model_without = create_lstm_model(input_shape, output_shape_without)
...
def calculate_aee_per_step(true_path, predicted_path, segments):
    ...
    for seg in segments:
        start_idx = seg[0]
        end_idx = seg[-1]
        true_segment = true_path[start_idx:end_idx+1, :2]
        predicted_segment = predicted_path[start_idx:end_idx+1]
        segment_errors = np.sqrt(np.sum((true_segment - predicted_segment) ** 2, axis=1))
        errors.append(segment_errors)
        true_flat.append(true_segment)
        pred_flat.append(predicted_segment)
    aee_per_step = np.concatenate(errors).mean()
    true_flat = np.concatenate(true_flat, axis=0)
    pred_flat = np.concatenate(pred_flat, axis=0)
    return aee_per_step, true_flat, pred_flat
...
if predicted_path is not None:
    # 计算AEE per step
    aee_per_step, true_flat, pred_flat = calculate_aee_per_step(taxi7, predicted_path, segment_paths(taxi7))
    print(f"AEE per step: {aee_per_step}")

    # 计算段长度分布
    true_segment_lengths = calculate_segment_lengths(taxi7, segment_paths(taxi7))
    predicted_segment_lengths = calculate_segment_lengths(predicted_path, segment_paths(taxi7))
    ...
    aee_errors = np.sqrt(np.sum((true_flat - pred_flat) ** 2, axis=1))
