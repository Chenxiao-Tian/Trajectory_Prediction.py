# -*- coding: utf-8 -*-
"""
Created on Fri Jun 28 11:47:36 2024
...
# 预测路径
def predict_path_xgboost(start, end, similar_segments, segment_length):
    ...
    xgb_lat = XGBRegressor(objective='reg:squarederror')
    xgb_lon = XGBRegressor(objective='reg:squarederror')
    xgb_lat.fit(X_train, y_train_lat)
    xgb_lon.fit(X_train, y_train_lon)

    X_pred = np.linspace(0, 1, segment_length).reshape(-1, 1)
    predicted_lat = xgb_lat.predict(X_pred)
    predicted_lon = xgb_lon.predict(X_pred)
...
for seg_indices in taxi7_segments:
    ...
    predicted_path = predict_path_xgboost(start, end, similar_segments, segment_length)
