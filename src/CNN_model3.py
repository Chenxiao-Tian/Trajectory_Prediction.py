# -*- coding: utf-8 -*-
"""
Created on Tue Jun 25 22:40:17 2024
...
def predict_trajectory(model, scaler, data, max_length):
    predictions = []
    for segment in segment_paths(data):
        start = data[segment[0], :2]
        end = data[segment[-1], :2]
        passenger_status = data[segment[0], 2]
...
def train_and_predict(data_with_passenger, data_without_passenger, scaler, max_length, epochs=20):
    ...
    input_shape = X_with.shape[1]
    output_shape = y_with.shape[1:]

    model_with = create_cnn_model(input_shape, output_shape)
    model_without = create_cnn_model(input_shape, output_shape)
    ...
    pred_with = predict_trajectory(model_with, scaler, np.array(taxi7), max_length)
    pred_without = predict_trajectory(model_without, scaler, np.array(taxi7), max_length)

    return pred_with, pred_without

# Load taxi data# Load taxi data
taxi_files = [
    'taxi1.txt',
    'taxi2.txt',
    'taxi3.txt',
