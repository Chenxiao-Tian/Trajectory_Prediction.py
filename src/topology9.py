                ...
                for i in range(start_idx, end_idx + 1):
                    if i == start_idx:
                        ...
                        if data[start_idx, 2] == 0:
                            diffusion = sigma * 1.2 * np.sqrt(dt) * np.random.normal(size=2)
                        else:
                            diffusion = sigma * 0.8 * np.sqrt(dt) * np.random.normal(size=2)
...
# 设置x轴的最大范围
max_segment_length = max(max(true_segment_lengths), max(predicted_segment_lengths))
