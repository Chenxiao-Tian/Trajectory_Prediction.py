# -*- coding: utf-8 -*-
"""
Created on Sun Jun 23 20:04:27 2024
...
# 将预测路径对齐到道路网络
snapped_predicted_paths = snap_to_nearest_road(G, predicted_paths)
...
aee_per_step = calculate_aee_per_step(taxi7, snapped_predicted_paths, taxi7_segments)
print(f"AEE per step: {aee_per_step}")

# 计算段长度分布
true_segment_lengths = calculate_segment_lengths(taxi7, taxi7_segments)
predicted_segment_lengths = calculate_segment_lengths(snapped_predicted_paths, taxi7_segments)
...
plt.plot(snapped_predicted_paths[:, 1], snapped_predicted_paths[:, 0], label='Predicted Path', color='red', alpha=0.6)
...
aee_errors = np.sqrt(np.sum((taxi7[:, :2] - snapped_predicted_paths) ** 2, axis=1))
