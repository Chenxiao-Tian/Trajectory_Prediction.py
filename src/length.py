# -*- coding: utf-8 -*-
"""
Created on Thu Jun 13 16:27:10 2024
...
# -*- coding: utf-8 -*-
"""
Created on Thu Jun 13 15:54:04 2024
...
def segment_based_simulation(data, segments, sigma, mu):
    ...
    for j in range(100):
        for seg in segments:
            ...
            x0 = data[start_idx, :2]
            xT = data[end_idx, :2]
            segment_length = 0.1*np.linalg.norm(xT - x0)
            for i in range(start_idx, end_idx + 1):
                if i == start_idx:
                    predicted_paths[j, i] = x0
                else:
                    t = (i - start_idx) * dt
                    predicted_paths[j, i] = x0 + t / segment_length * (xT - x0) + sigma * np.sqrt(t * (segment_length - t)) * np.random.normal(size=2)
...
def calculate_segment_lengths(path, segments):
    ...
    for seg in segments:
        start_idx = seg[0]
        end_idx = seg[-1]
        segment_length = np.sum(np.sqrt(np.sum(np.diff(path[start_idx:end_idx+1], axis=0)**2, axis=1)))
...
plt.hist(predicted_segment_lengths, bins=30, alpha=0.5, label='Predicted Path Segment Lengths', density=True)
