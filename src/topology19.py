# -*- coding: utf-8 -*-
"""
Created on Sat Jul 20 19:40:48 2024
...
for j in range(snapped_predicted_paths_bridge.shape[0]):
...
for j in range(1):  # Reduced the number of plotted snapped paths to 1 for clarity
...
aee_per_step = aee_bridge
aee_per_step2 = aee_brownian
print(f"Average AEE per step bridge: {np.nanmean(aee_per_step)}")
print(f"Average AEE per step brownian: {np.nanmean(aee_per_step2)}")
# Plot AEE per step
plt.figure(figsize=(10, 6))
...
plt.show()
# 反转timestamps和对应的AEE数据
# timestamps_reversed = timestamps[::-1]
# aee_brownian_reversed = aee_brownian[::-1]
# aee_bridge_reversed = aee_bridge[::-1]

# plt.plot(timestamps_reversed, aee_brownian_reversed, label='AEE (Brownian Motion)', color='green')
# plt.plot(timestamps_reversed, aee_bridge_reversed, label='AEE (Segmented Brownian Bridge)', color='red')
# plt.xlabel('Timestamp')
# plt.ylabel('AEE')
# plt.title('AEE per Step for Segmented Brownian Bridge and Brownian Motion')
# plt.legend()
