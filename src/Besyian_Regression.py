# -*- coding: utf-8 -*-
"""
Created on Fri Jun 28 11:38:28 2024
...
# 预测路径
def predict_path_bayesian(start, end, similar_segments, segment_length):
    ...
    bayesian_lat = BayesianRidge()
    bayesian_lon = BayesianRidge()
    bayesian_lat.fit(X_train, y_train_lat)
    bayesian_lon.fit(X_train, y_train_lon)

    X_pred = np.linspace(0, 1, segment_length).reshape(-1, 1)
    predicted_lat = bayesian_lat.predict(X_pred)
    predicted_lon = bayesian_lon.predict(X_pred)
    
    predicted_path = np.vstack((predicted_lat, predicted_lon)).T
...
for seg_indices in taxi7_segments:
    ...
    predicted_path = predict_path_bayesian(start, end, similar_segments, segment_length)
