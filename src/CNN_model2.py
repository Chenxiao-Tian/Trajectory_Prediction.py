# -*- coding: utf-8 -*-
"""
Created on Tue Jun 25 22:27:11 2024
...
def predict_trajectory(model, scaler, data, max_length):
    predictions = []
    for segment in segment_paths(data):
        segment_data = np.array(data[segment])[:, :2]
        segment_scaled = scaler.transform(segment_data)
...
# Train and predict
def train_and_predict(data_with_passenger, data_without_passenger, scaler, max_length, epochs=1):
...
# Train and predict
pred_with, pred_without = train_and_predict(taxi_data_with_passenger, taxi_data_without_passenger, scaler, max_length, epochs=1)
